...
# Custom CSS - Enhanced with colored badges
st.markdown("""
<style>
    ...
    .low-52w-badge {background-color: #DC2626; color: white; padding: 0.2rem 0.5rem; border-radius: 0.3rem; font-size: 0.8rem; font-weight: bold;}
    .low-26w-badge {background-color: #F59E0B; color: white; padding: 0.2rem 0.5rem; border-radius: 0.3rem; font-size: 0.8rem; font-weight: bold;}
    .both-lows-badge {background-color: #991B1B; color: white; padding: 0.2rem 0.5rem; border-radius: 0.3rem; font-size: 0.8rem; font-weight: bold; animation: pulse 2s infinite;}
    .high-52w-badge {background-color: #10B981; color: white; padding: 0.2rem 0.5rem; border-radius: 0.3rem; font-size: 0.8rem; font-weight: bold;}
    @keyframes pulse {
        0%, 100% { opacity: 1; }
        50% { opacity: 0.7; }
    }
...
def send_whatsapp_alert(stock_data, alert_type, low_threshold, high_threshold, portfolio_stocks):
    ...
    try:
        ...
        if not all([account_sid, auth_token, from_whatsapp, to_whatsapp]):
            st.warning("⚠️ Twilio credentials not configured in secrets.")
            return False
...
def main():
    ...
    if st.session_state.last_check_time:
        ...
        if minutes_since_check < 10:
            st.info(f"ℹ️ Data is cached (refreshed {minutes_since_check} min ago). Yahoo Finance has rate limits - please wait 10 minutes between refreshes for best results.")
    ...
    with st.sidebar:
        ...
        # Build available stocks list for portfolio selection
        if stock_mode == "NIFTY 50":
            available_stocks = get_nifty50_symbols()
        elif stock_mode == "NIFTY 50 + Custom":
            available_stocks = sorted(list(set(custom_stocks + get_nifty50_symbols())))
        else:
            available_stocks = sorted(custom_stocks)
        
        # Portfolio stocks (multi-select dropdown)
        st.subheader("💼 Portfolio Stocks")
        st.caption("Select stocks you own - will get HIGH alerts")
        portfolio_stocks = st.multiselect(
            "Choose your portfolio stocks",
            options=available_stocks,
            default=[],
            help="Select stocks you own. Get alerts when they're near 52W high (selling opportunity)"
        )
        ...
        # Auto-refresh settings (always enabled)
        auto_refresh_enabled = True  # Always enabled
        refresh_interval = 5  # 1 minute for testing
        
        st.markdown("---")
        st.info("🔄 Auto-Refresh: Enabled (every 1 minute)")
        ...
        if st.session_state.last_check_time:
            ...
            if auto_refresh_enabled:
                elapsed = int((datetime.now() - st.session_state.last_check_time).total_seconds())
                remaining = (refresh_interval * 60) - elapsed
                
                if remaining > 0:
                    mins, secs = divmod(remaining, 60)
                    st.caption(f"⏱️ Next refresh in: {mins}m {secs}s")
                    st.caption("💡 Tip: Manually refresh page to update countdown")
                else:
                    st.caption("⏱️ Ready to refresh...")
    ...
    if failed_count > 0:
        st.warning(f"⚠️ {failed_count} stock(s) could not be loaded. Data may be unavailable or symbol invalid.")
    ...
    # ========================================
    # SUMMARY METRICS (ENHANCED WITH PROGRESS BARS)
    # ========================================
    ...
    with col2:
        pct_26w = (len(low_26_stocks) / len(stock_data_list) * 100) if stock_data_list else 0
        st.metric("🟠 Near 26W Low", len(low_26_stocks), f"{pct_26w:.0f}%")
        st.progress(pct_26w / 100)
    
    with col3:
        pct_52w = (len(low_52_stocks) / len(stock_data_list) * 100) if stock_data_list else 0
        st.metric("🔴 Near 52W Low", len(low_52_stocks), f"{pct_52w:.0f}%")
        st.progress(pct_52w / 100)
    
    with col4:
        pct_both = (len(low_both_stocks) / len(stock_data_list) * 100) if stock_data_list else 0
        st.metric("🔥 Both Lows", len(low_both_stocks), f"{pct_both:.0f}%")
        st.progress(pct_both / 100)
    
    with col5:
        pct_high = (len(high_stocks) / len(stock_data_list) * 100) if stock_data_list else 0
        st.metric("🟢 Portfolio High", len(high_stocks), f"{pct_high:.0f}%")
    ...
    with filter_col1:
        show_all = st.button("📊 All Stocks", use_container_width=True, type="secondary")
    with filter_col2:
        show_26w = st.button("🟠 26W Lows", use_container_width=True)
    with filter_col3:
        show_52w = st.button("🔴 52W Lows", use_container_width=True)
    with filter_col4:
        show_both = st.button("🔥 Both Lows", use_container_width=True)
    with filter_col5:
        show_highs = st.button("🟢 Portfolio Highs", use_container_width=True)
    with filter_col6:
        show_nifty = st.button("📈 NIFTY 50", use_container_width=True)
    
    # Determine which stocks to display based on filter
    ...
    if show_all:
        ...
        st.session_state.filter_view = '26w'
    elif show_52w:
        st.session_state.filter_view = '52w'
    elif show_both:
        st.session_state.filter_view = 'both'
    elif show_highs:
        st.session_state.filter_view = 'highs'
    elif show_nifty:
        st.session_state.filter_view = 'nifty'
        ...
        for stock in high_stocks:
            with st.expander(f"💰 {stock['name']} ({stock['symbol']}) - Portfolio", expanded=True):
                ...
                with col1:
                    ...
                    st.markdown(f"**52-Week High:** ₹{stock['high_52']:.2f}")
    ...
    if low_both_stocks:
        ...
        st.info(f"ℹ️ **{len(unique_symbols)} Stock(s) Near Low Levels**")
    else:
        st.success("✅ No stocks at alert levels")
    
    # ========================================
    # FILTERED STOCK VIEW (BASED ON QUICK FILTERS)
    # ========================================
    ...
    # Apply quick filter
    if st.session_state.filter_view == '26w':
        filtered_stocks = low_26_stocks
        st.info(f"🟠 **Showing {len(filtered_stocks)} stocks near 26-Week Low**")
    elif st.session_state.filter_view == '52w':
        filtered_stocks = low_52_stocks
        st.info(f"🔴 **Showing {len(filtered_stocks)} stocks near 52-Week Low**")
    elif st.session_state.filter_view == 'both':
        filtered_stocks = low_both_stocks
        st.warning(f"🔥 **Showing {len(filtered_stocks)} stocks near BOTH 26W & 52W Lows**")
    elif st.session_state.filter_view == 'highs':
        filtered_stocks = high_stocks
        st.success(f"🟢 **Showing {len(filtered_stocks)} portfolio stocks near 52W High**")
    elif st.session_state.filter_view == 'nifty':
        filtered_stocks = [s for s in stock_data_list if s['symbol'] in nifty50_list]
        st.info(f"📈 **Showing {len(filtered_stocks)} NIFTY 50 stocks**")
    else:
        filtered_stocks = stock_data_list
        st.info(f"📊 **Showing all {len(filtered_stocks)} stocks**")
    ...
    if filtered_stocks:
        for stock in filtered_stocks:
            ...
            # Alert badges (highest priority)
            if stock['d26_low'] <= low_alert_threshold and stock['d52_low'] <= low_alert_threshold:
                badges_html += '<span class="both-lows-badge">🔥 BOTH LOWS</span> '
            elif stock['d52_low'] <= low_alert_threshold:
                badges_html += '<span class="low-52w-badge">🔴 52W LOW</span> '
            elif stock['d26_low'] <= low_alert_threshold:
                badges_html += '<span class="low-26w-badge">🟠 26W LOW</span> '
            
            if is_portfolio and stock['d52_high'] <= high_alert_threshold:
                badges_html += '<span class="high-52w-badge">🟢 52W HIGH</span> '
            
            # Category badges
            if is_portfolio:
                badges_html += '<span class="portfolio-badge">💼 PORTFOLIO</span> '
            if is_nifty:
                badges_html += '<span class="nifty-badge">📊 NIFTY 50</span>'
    ...
    if not filtered_stocks:
        st.info("No stocks to display in the selected filter.")
    else:
    ...
    if auto_refresh_enabled and st.session_state.last_check_time:
        ...
        elapsed_seconds = int((datetime.now() - st.session_state.last_check_time).total_seconds())
        refresh_threshold = refresh_interval * 60  # Convert to seconds
        
        if elapsed_seconds >= refresh_threshold:
            # Time to refresh - clear cache and rerun
            st.cache_data.clear()
            st.session_state.last_check_time = None
            st.rerun()
        else:
            # Not time yet - wait for remaining time then rerun
            remaining = refresh_threshold - elapsed_seconds
